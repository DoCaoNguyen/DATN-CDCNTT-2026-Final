...
**Data fields — audit_logs:**

| Trường | Kiểu dữ liệu | Bắt buộc | Mô tả |
|---|---|---|---|
| id | UUID/BIGINT | Có | Khóa chính |
| trace_id | String(100) | Không | ID truy vết request |
| actor_type | Enum | Có | USER / MERCHANT / ADMIN / SYSTEM |
...
**Data fields — system_logs:**

| Trường | Kiểu dữ liệu | Bắt buộc | Mô tả |
|---|---|---|---|
| id | UUID/BIGINT | Có | Khóa chính |
| trace_id | String(100) | Không | ID truy vết |
| level | Enum | Có | INFO / WARN / ERROR / CRITICAL |
...
| Bảng | Vai trò |
|---|---|
| `audit_logs` | Ghi log nghiệp vụ |
| `system_logs` | Ghi log kỹ thuật |
| `security_logs` | Có thể tách riêng hoặc dùng system_logs category security |
| `payment_orders` | Trace payment |
| `ledger_transactions` | Trace giao dịch |
| `ledger_entries` | Trace debit/credit |
| `payment_callbacks` | Trace callback |
| `users`, `merchants` | Actor liên quan |
| `app_settings` | Retention/mask config |
...
|---|---|---|---|
| O-01 | Tách riêng security_logs hay dùng system_logs? | Mở | MVP có thể dùng system_logs + category |
