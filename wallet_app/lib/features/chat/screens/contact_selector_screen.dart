...
class _ContactSelectorScreenState extends State<ContactSelectorScreen> {
  ...
  void _sendRedPacket() async {
    ...
    for (final phone in _selectedPhones) {
      socketService.sendMessage(phone, widget.redPacketId, messageType: 'RED_PACKET');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Chọn người nhận', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
        leading: IconButton(
      ...
      body: _isLoading
          ...
          : _chatList.isEmpty
              ? const Center(child: Text('Chưa có liên hệ nào', style: TextStyle(color: Colors.grey)))
              : ListView.builder(
        ...
        child: Container(
          ...
          child: SizedBox(
            ...
            child: ElevatedButton(
              onPressed: _isSending || _selectedPhones.isEmpty ? null : _sendRedPacket,
              ...
              child: _isSending
                  ...
                  : Text('Gửi (${_selectedPhones.length})', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.white)),
            ),
          ),
        ),
      ),
