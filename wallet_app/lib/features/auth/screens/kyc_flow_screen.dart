...
class _KycFlowScreenState extends State<KycFlowScreen> {
  ...
  Future<bool> _validateIdCardQuality(File imageFile, bool isFront) async {
    try {
      ...
      if (isFront) {
        ...
        bool hasBottom = text.contains("QUÊ QUÁN") || text.contains("THƯỜNG TRÚ") || text.contains("RESIDENCE") || text.contains("GIÁ TRỊ");

        return hasTop && hasTitle && hasId && hasBottom;
      } else {
        bool hasBackKeywords = text.contains("ĐẶC ĐIỂM") || text.contains("NHẬN DẠNG") || text.contains("CỤC TRƯỞNG") ||
            text.contains("GIÁM ĐỐC") || text.contains("CÔNG AN") || text.contains("NGÀY") || text.contains("THÁNG") || text.contains("NĂM");
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: const Row(
          children: [
            ...
            SizedBox(width: 8),
            Text("Ảnh không đạt chuẩn", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
          ],
        ),
        content: const Text(
        ...
        actions: [
          TextButton(
            ...
            child: const Text("Chụp lại", style: TextStyle(color: AppColors.primaryPink, fontWeight: FontWeight.bold, fontSize: 16)),
          ),
        ],
      ),
    );
  ...
  // --- HÀM GỌI API KIỂM TRA TRÙNG CCCD ---
  Future<bool> _isIdNumberUsed(String idNumber) async {
    try {
      // GỌI API TRỰC TIẾP LÊN BACKEND NODE.JS
      final response = await http.get(Uri.parse('${ApiConfig.baseUrl}/kyc/check-id?id=$idNumber'));
      ...
      print("Lỗi kiểm tra CCCD: $e");
    }
    
    return false; // Nếu gặp lỗi mạng tạm thời cho qua để không treo luồng
  }
  ...
  Future<void> _processOCR(File imageFile) async {
    try {
      ...
      // -- KIỂM TRA ĐIỀU KIỆN TRƯỚC KHI CHO QUA BƯỚC 3 --
      if (idNumber.isEmpty || dob.isEmpty) {
         setState(() => _isLoading = false);
         _showErrorDialog("Ảnh bị mờ hoặc bị che khuất. Không thể đọc được Số CCCD và Ngày sinh.");
         return;
      }

      // 1. Kiểm tra trùng lặp CCCD
      bool isUsed = await _isIdNumberUsed(idNumber);
      if (isUsed) {
        setState(() => _isLoading = false);
        _showErrorDialog("Số CCCD này đã được sử dụng cho một tài khoản khác. Vui lòng liên hệ CSKH.");
        return;
      }

      // 2. Kiểm tra đủ 18 tuổi
      if (!_isOver18(dob)) {
        setState(() => _isLoading = false);
        _showErrorDialog("Rất tiếc, bạn phải đủ 18 tuổi để sử dụng dịch vụ này.");
        return;
      }

      // Đủ điều kiện -> Cập nhật lên Giao diện Form và nhảy sang Bước 3
      setState(() {
      ...
      setState(() => _isLoading = false);
      _showErrorDialog("Lỗi quét dữ liệu: Vui lòng chụp lại ảnh.");
    }
  }
  ...
  // ====================================================================
  // GIAO DIỆN MÀN HÌNH FORM XÁC NHẬN OCR (BƯỚC 3) - KHÓA KHÔNG CHO SỬA
  // ====================================================================
        ...
        child: Column(
          ...
          children: [
            ...
            TextField(
              ...
              readOnly: true, // KHÓA SỬA
              decoration: InputDecoration(
                labelText: "Số CCCD",
                border: const OutlineInputBorder(),
                filled: true,
                fillColor: Colors.grey.shade100, // Làm nền xám để user biết là đã khóa
              ),
            ),
            const SizedBox(height: 16),
            TextField(
