...
class _KycFlowScreenState extends State<KycFlowScreen> {
  ...
  Future<void> _initializeCamera() async {
    ...
    if (_cameras != null && _cameras!.isNotEmpty) {
      // Sang bước 4 mới dùng Cam trước
  ...
  Future<void> _confirmPhoto() async {
    if (_currentStep == 1) {
      ...
      setState(() {
        ...
        _isPreviewing = false;
        _isCameraInitialized = false;
        _isLoading = true; // Hiện vòng xoay chờ đọc OCR
      });
      
      // Gọi hàm đọc chữ từ mặt trước CCCD
      await _processOCR(_idFrontImage!);
      
      setState(() {
        _isLoading = false;
        _currentStep = 3; // Chuyển sang màn hình Form Xác Nhận
      });
    }
  }

  // ====================================================================
  // BỘ NÃO ĐỌC CHỮ OCR (ML KIT OFFLINE)
  // ====================================================================
  // ====================================================================
  // BỘ NÃO ĐỌC CHỮ OCR (ĐÃ NÂNG CẤP BỘ LỌC REGEX)
  // ====================================================================
  Future<void> _processOCR(File imageFile) async {
    try {
      ...
      if (mounted) {
        showDialog(
          ...
          builder: (ctx) => AlertDialog(
            title: const Text("🔍 RAW TEXT TỪ AI", style: TextStyle(fontWeight: FontWeight.bold)),
            ...
            actions: [
              TextButton(
                ...
                child: const Text("ĐÓNG", style: TextStyle(color: Colors.pink, fontWeight: FontWeight.bold)),
              )
            ],
          ),
        );
      ...
      for (int i = 0; i < lines.length; i++) {
        ...
        if (fullName.isEmpty && RegExp(r'(TÊN|NAME|NARNE)').hasMatch(upperLine)) {
        ...
        if (gender.isEmpty && RegExp(r'(TÍNH|TINH|SEX)').hasMatch(upperLine)) {
          ...
          String cleanGenderLine = line.replaceAll(RegExp(r'(VIỆT NAM|VIET NAM)', caseSensitive: false), '');
          var gMatch = RegExp(r'(Nam|Nữ|Nu)', caseSensitive: false).firstMatch(cleanGenderLine);
          if (gMatch != null) {
        ...
        if (address.isEmpty && RegExp(r'(TRÚ|TRÙ|RESIDENCE)').hasMatch(upperLine)) {
          String val = line.replaceAll(RegExp(r'.*(TRÚ|TRÙ|RESIDENCE)[\s:/*]*', caseSensitive: false), '').trim();
          address = val;
          // Vét thêm dòng dưới (đề phòng địa chỉ bị cắt làm 2)
          if (i + 1 < lines.length) {
            String nextLine = lines[i + 1];
            // Bỏ qua nếu dòng dưới chỉ là 1 số rác (VD: số 2, số 6 trong ảnh của bạn)
            if (nextLine.length > 4 && !RegExp(r'^[0-9]+$').hasMatch(nextLine)) {
    ...
    _cameraController?.startImageStream((CameraImage image) async {
      ...
      try {
        ...
        if (faces.isNotEmpty) {
          ...
          double actualImageWidth = image.width < image.height ? image.width.toDouble() : image.height.toDouble();
  ...
  Future<void> _submitKycAPI() async {
    ...
    try {
      ...
      // Đóng gói thông tin chữ thành chuỗi JSON
      ...
      if (response.statusCode == 200) {
        ...
        try {
          ...
          _showErrorDialog(responseData['error'] ?? 'Lỗi xác thực: ${response.statusCode}');
        } catch (e) {
      ...
      builder: (_) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            const Text('Khuôn mặt và CCCD hoàn toàn trùng khớp. Hồ sơ của bạn đã được duyệt.', textAlign: TextAlign.center),
  ...
  String getLivenessInstruction() {
    if (_currentStep != 4) {
      return _isPreviewing ? "Kiểm tra ảnh có bị mờ hoặc lóa sáng không" : "Vui lòng đặt CCCD vừa khít vào khung hình chữ nhật";
    }
        ...
        child: Column(
          ...
          children: [
            ...
            TextField(
              ...
              decoration: const InputDecoration(labelText: "Họ và Tên (In hoa)", border: OutlineInputBorder()),
            ),
            const SizedBox(height: 16),
            TextField(
            ...
            TextField(
              ...
              decoration: const InputDecoration(labelText: "Quê quán / Nơi thường trú", border: OutlineInputBorder()),
              maxLines: 2,
            ),
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                style: ElevatedButton.styleFrom(backgroundColor: AppColors.primaryPink),
                onPressed: () {
                  if (_idNumberController.text.isEmpty || _fullNameController.text.isEmpty) {
                    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Vui lòng nhập đủ Số CCCD và Họ tên")));
                    return;
                  }
                  // Chuyển sang bước quét khuôn mặt
                  setState(() {
                    _currentStep = 4;
                    _isLoading = true; // Hiện loading 1 tẹo để mượt UX
                  });
                ...
                child: const Text('Xác nhận & Tiến hành quét khuôn mặt', style: TextStyle(fontSize: 16, color: Colors.white)),
              ),
            )
          ],
        ),
  ...
  Widget build(BuildContext context) {
    // Nếu đang ở Bước 3, hiển thị giao diện Form thay vì Camera
    if (_currentStep == 3) {
        ...
        body: Center(
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 16),
              Text('Đang xử lý dữ liệu...', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
            ],
          ),
        ),
      );
        ...
        children: [
          ...
          Positioned(
            ...
            child: Column(
              children: [
                Text(
                  isSelfieStep ? "XÁC THỰC KHUÔN MẶT" : "CHỤP MẶT ${(_currentStep == 1 ? "TRƯỚC" : "SAU")} CCCD",
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: isSelfieStep ? Colors.red.withOpacity(0.8) : Colors.black54,
                        ...
                        children: [
                          Expanded(
                            child: OutlinedButton(
                              ...
                              child: const Text('Chụp lại', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold)),
                            ),
                          ),
...
class CameraOverlayPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (isSelfie) {
      ...
      cutoutPath.addOval(Rect.fromCenter(
    ...
    final backgroundPath = Path.combine(PathOperation.difference, Path()..addRect(screenRect), cutoutPath);
