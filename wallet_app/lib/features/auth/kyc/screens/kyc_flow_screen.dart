...
class KycFlowScreen extends StatefulWidget {
  final String userId;
  final String token;
  const KycFlowScreen({Key? key, required this.userId, required this.token})
    : super(key: key);
...
class _KycFlowScreenState extends State<KycFlowScreen> {
  ...
  Future<void> _confirmPhoto() async {
    ...
    if (_currentStep == 1) {
      ...
      if (!isValidBack) {
        setState(() => _isLoading = false);
        KycDialogs.showWarning(
          context,
          message:
              "Mặt sau không hợp lệ hoặc không khớp với thẻ mặt trước. Vui lòng chụp cho khớp khung hình.",
        );
        _retakePhoto();
        return;
      }
  ...
  Future<bool> _processOCRFront(File idFrontImage) async {
    try {
      var request = http.MultipartRequest(
        'POST',
        Uri.parse('${ApiConfig.baseUrl}/kyc/ocr-front'),
      );
      request.files.add(
        await http.MultipartFile.fromPath('id_front', idFrontImage.path),
      );
      ...
      if (response.statusCode != 200) {
        ...
        KycDialogs.showError(
          context,
          json['error'] ?? "Lỗi phân tích thẻ CCCD",
          _resetFlow,
        );
        return false;
      }
      ...
      final checkResponse = await CustomHttpClient().get(
        Uri.parse('${ApiConfig.baseUrl}/kyc/check-id?id=${data["id"]}'),
      );
      if (checkResponse.statusCode == 200 &&
          jsonDecode(checkResponse.body)['is_used'] == true) {
        setState(() => _isLoading = false);
        KycDialogs.showError(
          context,
      ...
      KycDialogs.showError(
        context,
        "Lỗi quét dữ liệu: Vui lòng kiểm tra lại mạng hoặc chụp lại ảnh.",
        _resetFlow,
      );
      return false;
    }
  ...
  Future<void> _submitKycAPI() async {
    ...
    try {
      ...
      if (response.statusCode == 200)
        KycDialogs.showSuccess(context, widget.userId, widget.token);
      else
        KycDialogs.showError(
          context,
