...
class OcrHelper {
  static String removeDiacritics(String str) {
    ...
    for (int i = 0; i < withDia.length; i++) {
      ...
      str = str.replaceAll(
        withDia[i].toUpperCase(),
        withoutDia[i].toUpperCase(),
      );
    }
    return str;
  }

  static Future<bool> validateIdCardQuality(
    File imageFile,
    bool isFront, {
    String? expectedId,
    String? expectedName,
    ...
    try {
      ...
      if (isFront) {
        ...
        if (expectedName != null && expectedName.isNotEmpty) {
          String noDiaName = removeDiacritics(
            expectedName,
  ...
  static Future<Map<String, String>> extractInfo(File imageFile) async {
    ...
    for (int i = 0; i < lines.length; i++) {
      ...
      if (data["dob"]!.isEmpty && RegExp(r'(SINH|BIRTH)').hasMatch(upperLine)) {
        data["dob"] =
            RegExp(r'\d{2}[-/]\d{2}[-/]\d{4}').stringMatch(line) ?? "";
      }
      ...
      if (data["gender"]!.isEmpty &&
          RegExp(r'(TÍNH|TINH|SEX)').hasMatch(upperLine)) {
        var gMatch = RegExp(r'(Nam|Nữ|Nu)', caseSensitive: false).firstMatch(
          line.replaceAll(
        ...
        if (gMatch != null)
          data["gender"] = gMatch.group(1)!.toLowerCase() == 'nam'
              ? 'Nam'
              : 'Nữ';
      }
      if (data["address"]!.isEmpty &&
        ...
        if (i + 1 < lines.length &&
            lines[i + 1].length > 4 &&
            !RegExp(r'^[0-9]+$').hasMatch(lines[i + 1])) {
